...
class WSComposeMessageViewController: UIViewController {
    ...
    func configureAsReply(_ threadID: Int?) {
        ...
        guard let thread = try! store.retrieve(objectsWithClass: WSMOMessageThread.self, sortBy: nil, isAscending: true, predicate: predicate, context: store.managedObjectContext).first else { return }
