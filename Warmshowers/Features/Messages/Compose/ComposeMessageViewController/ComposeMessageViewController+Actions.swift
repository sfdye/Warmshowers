...
extension ComposeMessageViewController {
    ...
    @IBAction func sendButtonPressed(_ sender: AnyObject?) {
        ...
        var title = NSLocalizedString("Could not send message.", comment: "Title for the alert shown when a message could not be sent.")
        let button = NSLocalizedString("OK", comment: "OK button title")
        
        guard let body = body , body != "" else {
            let message = NSLocalizedString("Message has no content.", comment: "Message for the alert shown when a message could not be sent due to it having no content.")
            alert.presentAlertFor(self, withTitle: title, button: button, message: message)
            return
        }
        
        guard let recipients = recipients else {
            let message = NSLocalizedString("Message has no recipients.", comment: "Message for the alert shown when a message could not be sent due to it having no recipients.")
            alert.presentAlertFor(self, withTitle: title, button: button, message: message)
            return
        }
        
        guard let subject = subject , subject != "" else {
            let message = NSLocalizedString("Message has no subject.", comment: "Message for the alert shown when a message could not be sent due to it having no subject.")
            alert.presentAlertFor(self, withTitle: title, button: button, message: message)
            return
        }
        ...
        if isReply {
            ...
            guard let threadID = threadID else {
                ...
                let message = NSLocalizedString("Sorry, an error occured while trying to send your message. Please reload the view and try again.", comment: "Alert message when a reply to an unknown message is trying to be sent.")
                alert.presentAlertFor(self, withTitle: title, button: button, message: message)
                return
            }
        ...
        // Show the spinner
        let spinnerLabel = NSLocalizedString("Sending message ...", comment: "Message shown with the spinner while a message is being sent.")
