...
class LocationSearchViewController : UIViewController, Delegator, DataSource {
    ...
    @IBOutlet var statusLabel: MapLabel!
    ...
    override func viewWillAppear(_ animated: Bool) {
        assert(navigationDelegate != nil, "The navigation delegate for LocationSearchViewController not set. Please ensure the delegate is set before the view appears.")
    }
    ...
    func textForStatusLabel() -> String? {
        ...
        if downloadsInProgress.count > 0 {
            ...
            if highDensityArea {
                return NSLocalizedString("Updating ...\nThere are a lot of hosts in this area so this update may take a while.", tableName: "HostSearch", comment: "Status shown on the host search map")
            } else {
    ...
    func tilesInMapRegion(_ region: MKCoordinateRegion) -> Set<MapTile>? {
        ...
        guard var parentTiles = MapTile.tilesForMapRegion(mapView.region, atZoomLevel: minimumUpdateZoomLevel) else {
            return nil
        }
    ...
    func loadAnnotationsForMapTile(_ tile: MapTile) {
        
        func shouldDownloadHostsForMapTileMapTile(_ tile: MapTile) -> Bool {
...
//            print("Loading from the store.")
    ...
    func downloadWillStartForMapTile(_ tile: MapTile) {
        downloadsInProgress.insert(tile)
        dimUpdatingTiles()
    }
    
    /** Called after a user locations API request has finished. */
    func downloadDidEndForMapTile(_ tile: MapTile) {
