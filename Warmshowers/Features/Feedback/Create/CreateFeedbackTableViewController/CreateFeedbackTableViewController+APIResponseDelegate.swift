...
extension CreateFeedbackTableViewController : APIResponseDelegate {
    ...
    func request(_ request: APIRequest, didFailWithError error: Error) {
        DispatchQueue.main.async(execute: { [weak self] () -> Void in
            let title = NSLocalizedString("Could not submit feedback", tableName: "CreateFeedback", comment: "Alert title shown there was an API issue submitting feedback")
            let button = NSLocalizedString("OK", comment: "OK button title")
            let message = NSLocalizedString("Sorry, an error occured while submitting your feedback. Please check you are connected to the internet and try again later.", tableName: "CreateFeedback", comment: "Alert message shown there was an API issue submitting feedback")
