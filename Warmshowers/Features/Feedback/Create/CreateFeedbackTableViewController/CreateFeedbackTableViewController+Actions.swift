...
extension CreateFeedbackTableViewController {
    ...
    @IBAction func cancelButtonPressed(_ sender: AnyObject?) {
        ...
        if recommendation.hasBody {
            let message = NSLocalizedString("Are you sure you want to discard the current feedback?", comment: "Alert message for checking if the user wants to discard their drafted feedback")
    ...
    @IBAction func sumbitButtonPressed(_ sender: AnyObject?) {
        ...
        guard let _ = recommendation.recommendedUserName else {
            ...
            let message = NSLocalizedString("Recommended user not set. Please report this as a bug, sorry for the inconvenience.", comment: "Alert message shown with the recommended user is not set. This should never occur")
            alert.presentAlertFor(self, withTitle: title, button: button, message: message)
            return
        }
        
        guard recommendation.hasBody else {
            let title = NSLocalizedString("No feedback to submit", comment: "Alert title shown when the users tries to sumbit feedback before writing any")
            let button = NSLocalizedString("OK", comment: "OK button title")
            let message = NSLocalizedString("Please enter some feedback before submitting.", comment: "Alert message shown when the users tries to sumbit feedback before writing any")
            alert.presentAlertFor(self, withTitle: title, button: button, message: message)
            return
        }
        
        // Submit the feedback
        let message = NSLocalizedString("Submitting feedback ...", comment: "Label shown with the spinner while feedback is being submitted")
