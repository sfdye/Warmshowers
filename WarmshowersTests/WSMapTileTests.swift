...
//        var tile = MapTile(x: 9, y: 5, z: 3)
//        
//        // then
//        var tileExpectation: MapTile? = nil
...
//    }
    ...
    func testInitializer() {
        ...
        guard let tile = MapTile(x: 1501, y: 2885, z: 13) else {
            XCTFail("MapTile initializer failed.")
            return
        }
    ...
    func testConvenienceInitializer() {
        ...
        guard let tile = MapTile(latitude: 46.872193, longitude: -114.005724, zoom: 13) else {
            XCTFail("MapTile covenience initializer failed.")
            return
        }
        ...
        var largeLatitudeTile: MapTile? = MapTile(latitude: 90.0, longitude: -114.005724, zoom: 13)
        ...
        largeLatitudeTile = MapTile(latitude: -90.0, longitude: -114.005724, zoom: 13)
        
        // then
        XCTAssertNil(largeLatitudeTile, "Tile initialised for and out of bounds latitude. Initializer should have returned nil.")
        
        // when - longitude is out of the regular -180 - 180 degrees domain
        guard let largeLongitudeTile = MapTile(latitude: 45.0, longitude: 185.0, zoom: 3) else {
            XCTFail("MapTile covenience initializer failed.")
            return
        }
    ...
    func testTilesForMapRegion() {
        ...
        // when
        guard let tiles = MapTile.tilesForMapRegion(mapRegion, atZoomLevel: 3) else {
            XCTFail("MapTile covenience initializer failed.")
            return
        }
        ...
        XCTAssertEqual(tiles.count, countExpectation, "Incorrect number of tiles generated by MapTile.tilesForMapRegion factory method.")
    }
    ...
    func testMinimumLatitude() {
        ...
        // when
        guard let tile = MapTile(x: 15, y: 9, z: 4) else {
            XCTFail("MapTile covenience initializer failed.")
            return
        }
